  ...
  setup() {
    ...
    const loadOrders = async () => {
      try {
        ...
        const [fetchedOrders, submittedFetched] = await Promise.all([
        ...
        // Submitted orders shown separately, most recent first
        submittedOrders.value = submittedFetched.sort((a, b) => {
          const dateA = new Date(a.order_date)
          const dateB = new Date(b.order_date)
          return dateB - dateA
        })
    ...
    const getLeadTimeDays = (order) => {
      ...
      if (isNaN(orderDate.getTime()) || isNaN(expectedDate.getTime())) return 0
      return Math.round((expectedDate - orderDate) / 86400000)
    }
